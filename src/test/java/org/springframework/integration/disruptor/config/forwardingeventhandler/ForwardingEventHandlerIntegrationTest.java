package org.springframework.integration.disruptor.config.forwardingeventhandler;

import static org.junit.Assert.assertNotNull;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.integration.disruptor.ForwardingEventHandler;
import org.springframework.test.context.ContextConfiguration;
import org.springframework.test.context.junit4.SpringJUnit4ClassRunner;

@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration("/spring/forwarding-event-handler-test-config.xml")
public class ForwardingEventHandlerIntegrationTest {

	@Autowired
	private ForwardingEventHandler eventHandler;

	@Test
	public void config_ok() {
		assertNotNull(this.eventHandler);
	}

}
